...
import { useVendorDetail, useUpdateVendorStage } from '@/hooks/useVendor'
...
import { VendorOverview } from '@/components/modules/vendor/VendorOverview'
import { EngagementsTab } from '@/components/modules/vendor/EngagementsTab'
import type { VendorDetail, UpdateVendorStageInput } from '@/types/vendor'

const TABS = VENDOR_CONFIG.detailTabs
...
export default function VendorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, isLoading, isError } = useVendorDetail(id)
  const stageMutation = useUpdateVendorStage(id)

  const vendor = data?.result as VendorDetail | undefined
  const currentStage = vendor?.currentStage ?? ''
  const stageConfig = VENDOR_CONFIG.stages.find((s) => s.id === currentStage)
  ...
  async function handleStageUpdate(payload: { stage: string; notes?: string; drop_reason?: string }) {
    try {
      await stageMutation.mutateAsync(payload as UpdateVendorStageInput)
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-4">
        ...
        <Button variant="outline" onClick={() => router.push('/partnerships/vendors')}>Back to list</Button>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <button
        onClick={() => router.push('/partnerships/vendors')}
      ...
      <div className="flex gap-6 items-start">
              ...
              <div className="space-y-3">
                ...
                {vendor?.city && (
                  <div className="text-sm text-muted-foreground">
                    {vendor.city.name}, {vendor.state?.name}
                  </div>
                )}
              </div>
            )}
        ...
        <div className="flex-1 min-w-0">
          ...
          {!isLoading && vendor && activeTab === 'overview' && (
            <VendorOverview data={vendor} onTabChange={setActiveTab} />
          )}

          {activeTab === 'engagements' && vendor && (
            <EngagementsTab
              vendorId={id}
          ...
          {activeTab === 'school-tags' && (
            <div className="rounded-xl border border-dashed border-border p-8 text-center">
              <p className="text-sm text-muted-foreground">
                {vendor?.tagged_schools?.length
                  ? `${vendor.tagged_schools.length} linked school(s)`
      ...
      <StageUpdateModal
        ...
        partner={vendor ? { currentStage: vendor.currentStage } : {}}
